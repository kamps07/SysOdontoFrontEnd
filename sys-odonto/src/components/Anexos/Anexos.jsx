...
export default function Anexos({ anexos, adicionarAnexos, excluirAnexo }) {
    ...
    const handleFiles = (files) => {
        if (files.length > 0) {
            const anexosAdicionados = [];
            let filesProcessed = 0;

            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                if (file.size > 50 * 1024 * 1024) {
                    ToastService.Error("O arquivo é maior do que 50MB");
                    return;
                }
                const reader = new FileReader();
                reader.onload = (event) => {
                    const base64String = event.target.result;
                    anexosAdicionados.push({ file, base64String });

                    filesProcessed++;
                    if (filesProcessed === files.length) {
                        adicionarAnexos(anexosAdicionados);
                    }
                };
    ...
    return (
        ...
        >
            ...
            >
                ...
                {draggingFile > 0 &&
                    <span>Arraste e solte arquivos de até 50MB.</span>
                }
            </div>
        </div >
    )
